...
/**
 * Authoritative duplicate check against {@code companies} before registration.
 * <p>
 * Checks registration number first, then exact ACTIVE company name. Blocking JPA work runs on
 * {@link Schedulers#boundedElastic()} via {@link Mono}.
 */
@Service
public class CompanyDbLookupService {
    ...
    /**
     * @param companyRepository ACTIVE company lookups by registration number and name
     */
     ...
     * @return link, reject, or no-match outcome (async wrapper around blocking repository calls)
     */
